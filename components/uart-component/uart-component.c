    ...
    for (;;)
    {
        ...
        if (xQueueReceive(xQueue_UART0, (void *)&event, (TickType_t)portMAX_DELAY))
        {
            ...
            case UART_DATA:
                ...
                for (int i = 0; i < len; i++)
                {
                    if (data_uart[i] != char_SOP && index_packet == 0)
                        continue;
                    /* If we find the SOP */
...
#pragma GCC diagnostic pop
            ...
            /* Lamps. Flash UPLINK if set */
            lamps[0] = (lamps[0] == '1') ? '0' : '1';
            ltp305g_write_lamps(lamps);

            /* Output to Python Visualizer */
            sprintf(str_output, "%05d %05d %05d %05d %02d %02d\n",
                    res_dva, res_dvatx, res_dvb, res_dvbtx, pair_vn.verb, dsky_prog);
...
esp_err_t uart_begin()
{
    ...
    if (ret != ESP_OK)
        ESP_LOGE("UART", "Error installing UART0 driver");
    ret = uart_param_config(UART_NUM_0, &uart_config);
    if (ret != ESP_OK)
        ESP_LOGE("UART", "Error configuring UART0 driver");

    /* Create a task to handler UART event from ISR */
    xQueue_packets = xQueueCreate(20, sizeof(uart_pkt_t));
    if (xQueue_packets == NULL)
